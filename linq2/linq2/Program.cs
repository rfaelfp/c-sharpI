...
namespace linq2
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Category tier 1 sum prices " + r12);
            
            var r13 = products.Where(p => p.Category.Id == 1).Average(p => p.Price);
            Console.WriteLine("Category tier 1 average prices " + r13);

            var r14 = products.Where(p => p.Category.Id == 5).Select(p => p.Price).DefaultIfEmpty(0.0).Average();
            Console.WriteLine("Category tier 5 average prices without values " + r14);

            // Função de agragação, conhecido em outra linguagens como reduce
            var r15 = products.Where(p => p.Category.Id == 1).Select(p => p.Price).Aggregate(0.0, (x, y) => x + y);
            Console.WriteLine("Agreggation sum the values " + r15);
            Console.WriteLine();
